...
if [[ -x "${REPO_ROOT}/scripts/host/run_isaac_lab_training.sh" ]]; then
  if [[ -f /.dockerenv ]]; then
    ...
    echo "  ${REPO_ROOT}/scripts/host/run_isaac_lab_training.sh train --headless ${EXTRA_ARGS[*]}" >&2
    echo >&2
    echo "If Isaac Lab is not installed yet:" >&2
    echo "  ${REPO_ROOT}/scripts/host/install_isaac_lab.sh" >&2
    exit 1
  fi
  exec "${REPO_ROOT}/scripts/host/run_isaac_lab_training.sh" train --headless "${EXTRA_ARGS[@]}"
fi
