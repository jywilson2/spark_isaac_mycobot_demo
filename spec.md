...
The implementation must remain **sufficiently generic** to train a policy that transfers to a **physical MyCobot 280** in arbitrary reach scenarios: full-workspace target sampling, staged curriculum → demo → precision fine-tune, and validation at multiple episode horizons — not a single scripted pose.
...
### Direct approach near target (required)

When the end effector is inside the **approach zone** (default 50 mm of the target), motion must follow a **direct radial path** toward the goal. Lateral (corrective) steps are penalized in the reward during precision training stages so the policy learns to finish the reach without zig-zag hunting.
...
* **Perception Hardware:** RPi USB Camera (Phase 5+)
...
4. **Actions (6-dim joint space):** policy outputs **joint position deltas** (Δq, one per revolute joint), scaled and clipped; **soft joint limits** enforced. Raw actions are **EMA-smoothed** before application; a **jerk penalty** discourages abrupt step-to-step changes. **No IK solver** may appear in the control loop — the policy *is* the learned IK mapping.
5. **Reward:** potential-based distance reduction + time / action / jerk penalties + **lateral deviation penalty** inside the approach zone (precision stages) + terminal bonus inside tolerance.
6. **Curriculum:** staged target sampling — near current EE → medium annulus → full workspace.
7. **Success:** EE within **1 mm** of the target 3D position (`EE_REACH_TOLERANCE_M = 0.001`); early terminate on success. Early training stages may use a **coarse 25 mm** tolerance before precision fine-tune.
8. **Episode length:** default **30 s** for training **and** demo/play (`DEFAULT_EPISODE_LENGTH_S`).
9. **Training stop:** rolling reach success rate ≥ stage target or `--max-duration-minutes`. **Plateau abort is opt-in** (`--plateau-abort`; default off).
10. **Scripted multi-stage training (required):** `./scripts/run_staged_training.sh` executes the recipe in `isaac_lab/training_recipe.py` — coarse curriculum → demo → 5 mm direct-path precision → 1 mm final precision → demo verify. Stages must be **reproducible from scratch** without manual CLI steps.
11. **Demo validation:** after training, verify with `./scripts/verify_demo_policy.sh` at **20 / 30 / 40 s** horizons at **1 mm** tolerance; each must retain **≥ 95%** reach success.
12. **Training monitor:** `./scripts/monitor_training.sh --watch` tails the active host log and shows rolling reach metrics.
...
| Step | Command |
|------|---------|
| **Staged train + verify (recommended)** | `./scripts/host/run_isaac_lab_training.sh staged --headless` |
| Monitor training | `./scripts/host/run_isaac_lab_training.sh monitor --watch` |
...
| **Continuous GUI demo** | `./scripts/host/run_isaac_lab_training.sh demo` |
| **Demo regression (20/30/40 s @ 1 mm)** | `./scripts/host/run_isaac_lab_training.sh verify-demo --headless` |
| Full verify + integration train | `./scripts/host/verify_isaac_lab.sh --smoke-train` |

Cameras are **off** by default for Phase 2. EE cameras activate only with `--use-red-block-vision` (Phase 5).

---

## Phase 5 — Red-block vision + contact-and-push (opt-in)

Deferred from Phase 2. Enabled **only** with `--use-red-block-vision` (requires `--enable_cameras`).

| Component | Location |
|-----------|----------|
| Red threshold detector | `isaac_lab/phase5_red_block/block_vision.py` |
| Depth → base-frame localization | `isaac_lab/phase5_red_block/block_localization.py` |
| Init scan sequence | `isaac_lab/phase5_red_block/init_scan.py` |
| Contact-and-push env | `isaac_lab/phase5_red_block/red_block_env.py` |
| ROS tracker (mock/live) | `spark_verify_pkg/spark_verify_nodes/block_vision_tracker.py` |

Phase 5 task: locate red block via vision, move EE to target, contact, push 5 mm. Train with:
...
| Train | `./scripts/host/run_isaac_lab_training.sh staged --headless` |
...
| [docs/project_status.md](docs/project_status.md) | Operational status after each commit |
| [last_prompt.md](last_prompt.md) | Append-only user prompt log |
...
2. **Prepend** the new prompt at the top.
3. Move the previous block under `# Old prompts:` preserving all `## BEGIN` / `## END` blocks.
...
* [x] MDP contract, DirectRLEnv, host delegation, PPO trainer
* [x] Smooth motion — EMA smoothing + jerk penalty
* [x] Direct-path shaping + 1 mm tolerance target
* [x] Scripted multi-stage training (`training_recipe.py`, `run_staged_training.sh`)
* [ ] **Verify ≥ 95% reach @ 1 mm** on 20/30/40 s demo horizons (staged from-scratch train)
...
* [x] Isolated modules under `isaac_lab/phase5_red_block/`
...
| Training recipe | `isaac_lab/training_recipe.py` |
| Phase 5 red-block env | `isaac_lab/phase5_red_block/red_block_env.py` |
