...
Curated reference material for the `spark_isaac_mycobot_demo` project: background on the
MyCobot 280 hardware, the ROS 2 / Isaac ecosystem the pipeline is built on, and the specific
concepts (NITROS, launch_testing, ONNX, edge deployment) used throughout `spark_verify_pkg`.
Each entry notes where in this repository the material applies.
